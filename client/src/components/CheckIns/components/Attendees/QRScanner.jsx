...
function QRScanner() {
  ...
  // Control scanner based on isScanning state
  useEffect(() => {
    if (scannerRef.current) {
      if (isScanning) {
        scannerRef.current.start().catch((err) => {
  ...
  const handleScan = async (result) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/attendees/validate`,
        { qrCodeData: result },
        {
  ...
  const toggleCamera = () => {
    ...
    setIsScanning(true); // Restart scanning with new camera
  };
      ...
      >
        <div className="bg-red-600 p-4 text-white">
          <div className="flex justify-between items-center">
            ...
            <Link
              to={`/checkins/events/${eventId}/attendees`}
