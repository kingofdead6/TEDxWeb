...
function AttendeesTables() {
  ...
  const [columns, setColumns] = useState(['fullName', 'email']);
  ...
  useEffect(() => {
    const fetchAttendees = async () => {
      try {
        const res = await axios.get(`${API_BASE_URL}/attendees/${eventId}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        });
        console.log('Fetched attendees:', res.data); // Debug
        setAttendees(res.data);
        setFilteredAttendees(res.data);
        const allColumns = new Set(['fullName', 'email']);
        res.data.forEach((attendee) => {
          Object.keys(attendee).forEach((key) => {
            if (!['id', 'createdAt', 'updatedAt', 'registrations', 'events', 'status', 'validationTime', 'qrCode'].includes(key)) {
              allColumns.add(key);
            }
          });
        });
  ...
  useEffect(() => {
    const filtered = attendees.filter((attendee) => {
      const nameMatch = attendee.fullName?.toLowerCase().includes(searchTerm.toLowerCase());
      const emailMatch = attendee.email?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    if (selectedFile) {
      ...
      reader.onload = (event) => {
        ...
        if (jsonData.length > 0) {
          const newColumns = Object.keys(jsonData[0]).filter(
            (key) => !['id', 'createdAt', 'updatedAt', 'registrations', 'events', 'status', 'validationTime', 'qrCode'].includes(key)
          );
          setColumns((prev) => {
  ...
  const handleUpload = async (selectedFile) => {
    ...
    try {
      const res = await axios.post(`${API_BASE_URL}/attendees/${eventId}/upload`, formData, {
        headers: {
      ...
      const attendeesRes = await axios.get(`${API_BASE_URL}/attendees/${eventId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
      ...
      console.error('Upload attendees error:', err.response?.data || err.message);
  ...
  const generateQRCode = async (attendee) => {
    try {
      const qrCodeData = JSON.stringify({ eventId, email: attendee.email });
      const qrCodeUrl = await QRCode.toDataURL(qrCodeData);
      const link = document.createElement('a');
      link.href = qrCodeUrl;
      link.download = `qr_code_${attendee.fullName.replace(/\s+/g, '_') || 'attendee'}.png`;
      ...
      >
        <div className="bg-[#e62b1e] p-4 text-white">
          <div className="flex justify-between items-center">
            ...
            <Link
              to={`/checkins/events/${eventId}/scan`}
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <div className="flex-shrink-0">{getStatusIcon(attendee.status)}</div>
                          <div className="ml-2">
                            <span
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {attendee.validationTime
                          ? new Date(attendee.validationTime).toLocaleString()
                          : 'Not checked in'}
                      </td>
